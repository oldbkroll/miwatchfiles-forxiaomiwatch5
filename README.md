...
正式版本 `0.3.1` 已完成签名构建并发布为 GitHub Release。
...
使用 Android Studio 打开项目根目录，在 Build Variants 中选择 `debug` 或 `release` 构建变体。
...
正式 Release 推荐使用 Android Studio 的 Generate Signed Bundle / APK 流程，选择 APK、`release` 构建变体和本机签名 keystore 完成构建。签名密码由 Android Studio 在本机流程中输入或管理，不要写入 README、脚本、Gradle 配置或 Git 仓库。
