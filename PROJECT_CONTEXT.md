...
普通文件详情、MIME 分类和系统打开已经实现。下一步建议：

1. 真机分别验证 JPEG/PNG、MP3、MP4 和未知文件。
2. 根据手表已有应用和系统选择器的实际表现修复兼容问题。
3. 如果手表缺少对应媒体应用，再决定是否内置轻量图片、音频或视频查看器。
4. 之后再加入新建文件夹、重命名和复制；删除功能最后加入，并要求二次确认。
...
然后检查工作树和构建状态，从阶段二继续，不要重新初始化 Android 项目，也不要撤销 target 29 和自定义表冠兼容逻辑。
