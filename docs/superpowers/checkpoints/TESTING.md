...
本版本已支持新建文件夹、重命名、复制和移动；删除确认仍留到 M1C。音频和视频播放器不在 M1B 范围。
...
当前版本已提供新建文件夹、重命名、复制和移动；删除确认尚未开放。所有写操作只在 `/storage/emulated/0/Download/WatchFilesTest/M1Sandbox` 中测试；不要直接使用重要照片、录音或系统目录。
