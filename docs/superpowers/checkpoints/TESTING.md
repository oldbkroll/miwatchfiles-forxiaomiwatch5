...
5. 在 M1Sandbox 内准备 UTF-8 空文本和多行 `.txt` 文件；点击后应进入文本查看/编辑页，文本内容、换行和返回目录位置保持正确。
6. 编辑文本并覆盖当前文件。保存前应有明确确认；保存成功后重新打开核对内容，保存失败、取消或应用中断时原文件内容不变，且没有残留 `.part` 或其他任务临时文件。
...
本版本已支持新建文件夹、重命名、复制、移动和带独立确认的递归永久删除；内置音频播放器不开发，音视频继续通过“用其他应用打开”，内置视频播放移至 M4 可选评估。
...
当前版本已提供新建文件夹、重命名、复制、移动和带独立确认的递归永久删除。文本覆盖和当前目录内另存为也必须只在 `/storage/emulated/0/Download/WatchFilesTest/M1Sandbox` 中测试；不要直接使用重要照片、录音或系统目录。开发期保持 Debug，Release 交接不在本检查点范围。
